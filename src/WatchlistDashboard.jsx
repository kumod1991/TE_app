    ...
    <>
      ...
      <div style={{ padding: isMobile ? "0 16px 0" : 18, overflowY: isMobile ? "auto" : "visible", flex: isMobile ? 1 : "none" }}>
            ...
            {SMA_FILTERS.map(({key,label}) => {
              ...
              return (
                <button key={key} onClick={()=>onChange(key, active?null:true)}
                  style={{ padding: isMobile ? "12px 10px" : "10px 10px", fontSize: isMobile ? 12 : 11, fontWeight:600,
        ...
        <div style={{ display:"flex", gap:10, flexWrap:"nowrap", position:isMobile ? "sticky" : "static", bottom:0, background:isMobile ? T.surface : "transparent", padding:isMobile ? "14px 0 calc(14px + env(safe-area-inset-bottom, 0px))" : 0, marginTop:isMobile ? 0 : 6, borderTop:isMobile ? `1px solid ${T.border}` : "none" }}>
          <button onClick={onApply} style={{ flex:1, padding: isMobile ? "12px 0" : "10px 0",
          ...
          <button onClick={onClear} style={{ flex:1, padding: isMobile ? "12px 0" : "10px 0",
                    ...
                    <div style={{ display:"flex", gap:4, alignItems:"center", marginTop:6, flexWrap:"nowrap", overflow:"hidden" }}>
                        ...
                        {showAllSignals && sortedScreens.length > MAX_VISIBLE && (
                            <span onClick={e => { e.stopPropagation(); setShowAllSignals(false); }}
                                style={{ fontSize:9, color:T.subtext, flexShrink:0, cursor:"pointer",
                                  fontFamily:"'DM Mono',monospace", opacity:0.5 }}>less</span>
                        )}
                    </div>
                );
            })()}
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:12}}>
          {watchlists.map(w=>(
            <button key={w.id} onClick={()=>toggle(w.id)}
              style={{padding:"3px 11px",borderRadius:14,fontSize:11,cursor:"pointer",
                background:sel.includes(w.id)?`${T.green}18`:"transparent",
                color:sel.includes(w.id)?T.green:T.subtext,
                border:`1px solid ${sel.includes(w.id)?T.green:T.border}`}}>{w.name}</button>
          ))}
        </div>
...
export default function WatchlistDashboard({ T, session, darkMode: darkModeProp, onToggleDark, onNavigateToScreen, onTechnoFunda,
  ...
  useEffect(() => {
    ...
    const params={watchlistId:activeWl,token,page,pageSize:PAGE_SIZE,sortCol,sortAsc,
  ...
  useEffect(()=>{
    if(!activeWl)return;
    const nextParams={watchlistId:activeWl,token,page:page+1,pageSize:PAGE_SIZE,sortCol,sortAsc,
      filters:quickFilter?{...filtersApplied,quick:quickFilter}:filtersApplied};
                          ...
                          <div style={{ display:"flex", flexWrap:"wrap", gap:4 }}>
                            {screenMembership[expandedRow.ticker].map(s=>(
                              <span key={s}
                                ...
                                style={{ fontSize:9, padding:"2px 6px", borderRadius:3, border:`1px solid ${T.border}`, color:T.subtext,
    ...
    <div
        ...
        <div style={{ display:"flex", gap: isMobile ? 6 : 3 }}>
          {FILTERS.map(f=>(
            <button key={f.key} onClick={()=>setFilter(f.key)}
              style={{ padding: isMobile ? "6px 14px" : "3px 10px", fontSize: isMobile ? 13 : 13,
      ...
      <div ref={scrollRef} style={{ flex:1, overflowY:"auto" }}>
        {loading ? (
          ...
          <div key={dateLabel}>
            {/* Date separator */}
            <div style={{ padding:"8px 16px 4px", position:"sticky", top:0, zIndex:2,
              ...
              return (
                ...
                >
                  ...
                  <div style={{ marginBottom:4 }}>
                    <span style={{ fontSize:12, fontWeight:600, padding:"2px 7px", borderRadius:3,
    ...
    <div
      ...
      <div style={{ flexShrink: 0, padding: isMobile ? "10px 18px 12px" : "14px 16px 10px", borderBottom: `1px solid ${T.border}` }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: isMobile ? 12 : 10 }}>
          ...
          <button onClick={onClose}
            style={{ background: "none", border: `1px solid ${T.border}`, borderRadius: isMobile ? 8 : 4, cursor: "pointer",
        ...
        <div style={{ display: "flex", gap: isMobile ? 6 : 4, marginBottom: isMobile ? 12 : 8 }}>
          {[["all", "All Companies"], ["watchlist", "My Watchlist"]].map(([k, l]) => (
            <button key={k} onClick={() => setMode(k)}
              style={{ padding: isMobile ? "6px 14px" : "3px 10px", fontSize: 13, fontWeight: 500, borderRadius: 20, cursor: "pointer",
          ...
          <>
            {grouped.entries.map(([dateKey, items]) => {
              ...
              return (
                <div key={dateKey}>
                  {/* Date group header */}
                  <div style={{ padding: "8px 16px 5px", position: "sticky", top: 0, zIndex: 2,
                    background: T.surface, borderBottom: `1px solid ${T.border}`,
                    display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <span style={{ fontSize: 12, fontWeight: 600, color: T.subtext,
                      ...
                      >
                        ...
                        {(item.exchange || item.market_cap_category) && (
                          <div style={{ display: "flex", gap: 6, marginTop: 4, flexWrap: "wrap" }}>
                            {item.exchange && (
                              <span style={{ fontSize: 11, fontWeight: 500, padding: "1px 5px", borderRadius: 3,
